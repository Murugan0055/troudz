    ...
    <nav className={`w-screen absolute z-40 md:text-sm ${className}`}>
      <div className={`items-center md:flex md:px-8 px-4`}>
        <div className="flex items-center justify-between py-3 md:py-5 md:block">
          <Link to="/">
            <img src={logo} width={40} height={40} alt="Float UI logo" />
          </Link>
          <div className="md:hidden">
            <button
              className="text-white text-2xl bg-none"
        ...
        <div
          className={`flex-1 pb-3 md:block md:pb-0 md:mt-0 ${
            sidebarOpen
              ? "absolute block bg-black z-30 w-2/3 h-screen top-0 right-0 mt-0 px-4 py-3"
              : "hidden"
          }`}
        >
          {sidebarOpen ? (
            <div className="flex justify-end px-5 pt-2">
              <button
                className="text-white text-xl bg-none"
                onClick={() => setSidebarOpen(!sidebarOpen)}
              >
          ...
          <ul
            className={`justify-end items-center space-y-6 md:flex md:space-y-0  mt-4 ${
              sidebarOpen ? "px-2" : ""
            }`}
          >
              ...
              return (
                <li key={idx} className="flex justify-center md:justify-start">
                  <Button
                    ...
                    className={`bg-gradient-to-r from-[#006CFC] to-[#00DDE8] bg-clip-text hover:text-transparent text-md md:text-lg font-medium ${
